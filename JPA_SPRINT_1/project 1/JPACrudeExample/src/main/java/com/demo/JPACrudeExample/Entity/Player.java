package com.demo.JPACrudeExample.Entity;
...
@Entity
@Table(name="player")
//@NamedQuery(name="Player.findByEmail", query=" SELECT p from Player p WHERE p.emailAddress = :email")
@NamedQuery(name="Player.findByFuzzyEmail", query="SELECT p from Player p WHERE p.emailAddress like :email")
public class Player {
	@Id
	@GeneratedValue
	private Long id;
	...
	public void setRegistrationDate(Calendar string) {
	...
	public void setRegistrationDate(int i, int j, int k) {
